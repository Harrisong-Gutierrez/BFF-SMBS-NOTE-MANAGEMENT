...
const NoteEditor: React.FC<NoteEditorProps> = ({ notes, setNotes }) => {
  const [newNote, setNewNote] = useState<string>('');
  const [editIndex, setEditIndex] = useState<number | null>(null); // Índice de la nota que se está editando

  const handleAddNote = () => {
    if (newNote.trim() !== '') {
      if (editIndex !== null) {
        // Si se está editando una nota existente, actualiza la nota en lugar de añadir una nueva
